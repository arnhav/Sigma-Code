...
public class TypeConversion {
    private static final Charset f578a;

    static {
        f578a = Charset.forName("UTF-8");
    }
    ...
    public static byte[] stringToUtf8(String javaString) {
        byte[] bytes = javaString.getBytes(f578a);
        if (javaString.equals(new String(bytes, f578a))) {
            return bytes;
        }
        throw new IllegalArgumentException(String.format("string cannot be cleanly encoded into %s - '%s' -> '%s'", new Object[]{f578a.name(), javaString, new String(bytes, f578a)}));
    }

    public static String utf8ToString(byte[] utf8String) {
        return new String(utf8String, f578a);
    }
}
