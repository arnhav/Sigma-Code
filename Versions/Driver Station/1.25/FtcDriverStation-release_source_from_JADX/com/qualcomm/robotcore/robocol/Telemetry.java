...
public class Telemetry implements RobocolParsable {
    ...
    private static final Charset f714a;
    private final Map<String, String> f715b;
    private final Map<String, Float> f716c;
    private String f717d;
    private long f718e;

    static {
        f714a = Charset.forName("UTF-8");
    }

    public Telemetry() {
        this.f715b = new HashMap();
        this.f716c = new HashMap();
        this.f717d = BuildConfig.VERSION_NAME;
        this.f718e = 0;
    }
    ...
    public synchronized boolean hasData() {
        boolean z;
        z = (this.f715b.isEmpty() && this.f716c.isEmpty()) ? false : true;
        return z;
    }
    ...
    public synchronized byte[] toByteArray() throws RobotCoreException {
        ByteBuffer allocate;
        this.f718e = System.currentTimeMillis();
        if (this.f715b.size() > Command.MAX_COMMAND_LENGTH) {
            throw new RobotCoreException("Cannot have more than 256 string data points");
        } else if (this.f716c.size() > Command.MAX_COMMAND_LENGTH) {
            ...
            allocate.putLong(this.f718e);
            if (this.f717d.length() == 0) {
                allocate.put((byte) 0);
            } else {
                byte[] bytes2 = this.f717d.getBytes(f714a);
                if (bytes2.length > Command.MAX_COMMAND_LENGTH) {
                    throw new RobotCoreException(String.format("Telemetry tag cannot exceed 256 bytes [%s]", new Object[]{this.f717d}));
                }
                allocate.put((byte) bytes2.length);
                allocate.put(bytes2);
            }
            allocate.put((byte) this.f715b.size());
            for (Entry entry : this.f715b.entrySet()) {
                bytes = ((String) entry.getKey()).getBytes(f714a);
                byte[] bytes3 = ((String) entry.getValue()).getBytes(f714a);
            ...
            allocate.put((byte) this.f716c.size());
            for (Entry entry2 : this.f716c.entrySet()) {
                bytes = ((String) entry2.getKey()).getBytes(f714a);
    ...
    public synchronized void fromByteArray(byte[] byteArray) throws RobotCoreException {
        ...
        synchronized (this) {
            ...
            if (unsignedByteToInt == 0) {
                ...
                wrap.get(bArr);
                this.f717d = new String(bArr, f714a);
            }
            byte b3 = wrap.get();
            for (b2 = (byte) 0; b2 < b3; b2++) {
                ...
                wrap.get(bArr3);
                this.f715b.put(new String(bArr2, f714a), new String(bArr3, f714a));
            }
            b2 = wrap.get();
            while (b < b2) {
                byte[] bArr4 = new byte[TypeConversion.unsignedByteToInt(wrap.get())];
                wrap.get(bArr4);
                this.f716c.put(new String(bArr4, f714a), Float.valueOf(wrap.getFloat()));
                b++;
            }
        }
    }
    ...
    private int m451a() {
        int length = (0 + (this.f717d.getBytes(f714a).length + 1)) + 1;
        int i = length;
        for (Entry entry : this.f715b.entrySet()) {
            i = (((String) entry.getValue()).getBytes(f714a).length + 1) + ((((String) entry.getKey()).getBytes(f714a).length + 1) + i);
        }
        length = i + 1;
        int i2 = length;
        for (Entry entry2 : this.f716c.entrySet()) {
            i2 = ((((String) entry2.getKey()).getBytes(f714a).length + 1) + i2) + 4;
        }
        return i2;
    }
}
