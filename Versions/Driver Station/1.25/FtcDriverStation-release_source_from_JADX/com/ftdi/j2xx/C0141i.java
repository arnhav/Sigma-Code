...
class C0141i extends C0009k {
    private static FT_Device f634d;

    C0141i(FT_Device fT_Device) {
        super(fT_Device);
        f634d = fT_Device;
    }

    boolean m412a(short s, short s2) {
        boolean z = false;
        int i = s2 & 65535;
        int i2 = s & 65535;
        if (s < D2xxManager.FT_FLOW_XON_XOFF) {
            byte latencyTimer = f634d.getLatencyTimer();
            f634d.setLatencyTimer((byte) 119);
            if (f634d.m28c().controlTransfer(64, 145, i, i2, null, 0, 0) == 0) {
                z = true;
            }
            f634d.setLatencyTimer(latencyTimer);
        }
        return z;
    }

    short m411a(FT_EEPROM ft_eeprom) {
        int[] iArr = new int[80];
        ...
        try {
            ...
            i2 = m43a(ft_eeprom_245r.Product, iArr, m43a(ft_eeprom_245r.Manufacturer, iArr, 12, 7, true), 8, true);
            if (ft_eeprom_245r.SerNumEnable) {
                m43a(ft_eeprom_245r.SerialNumber, iArr, i2, 9, true);
            }
    ...
    FT_EEPROM m410a() {
        ...
        try {
            ...
            m49a(ft_eeprom_245r, iArr[4]);
            m50a((Object) ft_eeprom_245r, iArr[5]);
            ...
            ft_eeprom_245r.Manufacturer = m47a(((iArr[7] & FT_4222_Defines.CHIPTOP_DEBUG_REQUEST) - 128) / 2, iArr);
            ft_eeprom_245r.Product = m47a(((iArr[8] & FT_4222_Defines.CHIPTOP_DEBUG_REQUEST) - 128) / 2, iArr);
            ft_eeprom_245r.SerialNumber = m47a(((iArr[9] & FT_4222_Defines.CHIPTOP_DEBUG_REQUEST) - 128) / 2, iArr);
    ...
    int m414b() {
        return (((63 - ((((((m44a((short) 7) & 65280) >> 8) / 2) + 12) + (((m44a((short) 8) & 65280) >> 8) / 2)) + 1)) - (((m44a((short) 9) & 65280) >> 8) / 2)) - 1) * 2;
    }

    int m409a(byte[] bArr) {
        if (bArr.length > m414b()) {
            return 0;
        }
        int[] iArr = new int[80];
        for (short s = (short) 0; s < (short) 80; s = (short) (s + 1)) {
        ...
        int b = (short) (((short) ((63 - (m414b() / 2)) - 1)) & 65535);
        int i = 0;
        while (i < bArr.length) {
            int i2;
            if (i + 1 < bArr.length) {
